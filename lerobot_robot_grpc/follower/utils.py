"""
Shared protobuf <-> Python (de)serialization helpers used by both the gRPC client
(`GRPCFollower`) and the server (`FollowerServicer`). Keep this module free of
client/server logic so a change only ever needs to be made in one place.
"""
...
# Python scalar types are exposed in `observation_features`/`action_features`: values parsed
